  ...
  const initConnection = useCallback(() => {
    ...
    try {
      ...
      socket.onopen = () => {
        ...
        // Re-subscribe to each match after reconnect using the ONLY protocol your backend supports
        for (const id of subscribedMatchIdsRef.current) {
          socket.send(
            JSON.stringify({ type: "subscribe", matchId: Number(id) }),
          );
        }
